...
static int init(VmafFeatureExtractor *fex, enum VmafPixelFormat pix_fmt,
    ...
    CHECK_CUDA(cu_f, cuModuleGetFunction(&cu_s->preprocess_nodither_u16, module, "preprocess_nodither_uint16_t_true"));
...
static void decimate_and_filter_mode(CudaFunctions *cu_f, CambiCudaState *cu_s,
    ...
    bool decimate = !(image->w[0] == scaled_width && image->h[0] == scaled_height);
    if (decimate) {
...
static int cambi_score(CudaFunctions *cu_f, CambiState *s, VmafPicture *pics, uint16_t window_size, double topk,
                       const uint16_t num_diffs, uint16_t vlt_luma,
