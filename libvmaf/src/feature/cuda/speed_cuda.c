...
static int compute_speed_params(VmafFeatureExtractor *fex, SpeedCudaState *cu_s,
  ...
  // #region agent log - Hypothesis E,F: GPU after linear solve
  {
    float indep_vals[4], solution_vals[4];
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(indep_vals, indep_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(solution_vals, solution_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuStreamSynchronize(cu_s->str));
    FILE *logf = fopen("/tmp/tmp.itUGhU69br/libvmaf/debug.log", "a");
    if (logf) {
      fprintf(logf,
              "{\"hypothesisId\":\"E_F\",\"location\":\"speed_cuda.c:after_linear_solve\","
              "\"message\":\"GPU after linear solve\",\"data\":{"
              "\"indep_first_4\":[%f,%f,%f,%f],"
              "\"solution_first_4\":[%f,%f,%f,%f]"
              "},\"timestamp\":%ld}\n",
              indep_vals[0], indep_vals[1], indep_vals[2], indep_vals[3],
              solution_vals[0], solution_vals[1], solution_vals[2], solution_vals[3],
              (long)time(NULL));
      fclose(logf);
    }
  }
...
extract_channel_cuda_async(VmafFeatureExtractor *fex, SpeedState *speed_state,
  ...
  // int op_h = dim->operating_height;
  // static int index = 0;
  // CUdeviceptr temp_ptr;
  // size_t frame_size = op_h * stride_px * sizeof(float);
  // void *frame_cpu = malloc(frame_size);
  // // copy image data back to host for debugging
  // {
  //   vmaf_cuda_buffer_get_dptr(bufs->frame_ref, &temp_ptr);
  //   CHECK_CUDA(cu_f,
  //              cuMemcpyDtoHAsync(frame_cpu, temp_ptr, frame_size, cu_s->str));
  //   CHECK_CUDA(cu_f, cuStreamSynchronize(cu_s->str));
  //   // write_buffer_as_image("preprocess_ref_gpu", index, op_w, op_h,
  //   //                       stride_px * sizeof(float), 32, frame_cpu);
  // }
  ...
  // {
  //   vmaf_cuda_buffer_get_dptr(bufs->frame_dis, &temp_ptr);
  //   CHECK_CUDA(cu_f,
  //              cuMemcpyDtoHAsync(frame_cpu, temp_ptr, frame_size, cu_s->str));
  //   CHECK_CUDA(cu_f, cuStreamSynchronize(cu_s->str));
  //   // write_buffer_as_image("preprocess_dis_gpu", index, op_w, op_h,
  //   //                       stride_px * sizeof(float), 32, frame_cpu);
  // }
  // free(frame_cpu);
  // ++index;
  ...
  // #region agent log - Hypothesis A,B,C: GPU est_params results
  {
    CUdeviceptr eigen_ptr, ref_var_ptr, ref_ent_ptr, dis_var_ptr, dis_ent_ptr, cov_ptr;
    vmaf_cuda_buffer_get_dptr(bufs->eigenvalues, &eigen_ptr);
    vmaf_cuda_buffer_get_dptr(bufs->variances_ref, &ref_var_ptr);
    vmaf_cuda_buffer_get_dptr(bufs->entropies_ref, &ref_ent_ptr);
    vmaf_cuda_buffer_get_dptr(bufs->variances_dis, &dis_var_ptr);
    vmaf_cuda_buffer_get_dptr(bufs->entropies_dis, &dis_ent_ptr);
    vmaf_cuda_buffer_get_dptr(bufs->cov_matrix, &cov_ptr);
    float eigenvalues[4], ref_variances[4], ref_entropies[4];
    float dis_variances[4], dis_entropies[4], cov_diag[5];
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(eigenvalues, eigen_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(ref_variances, ref_var_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(ref_entropies, ref_ent_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(dis_variances, dis_var_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(dis_entropies, dis_ent_ptr,
                                        4 * sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(&cov_diag[0], cov_ptr + 0 * sizeof(float), sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(&cov_diag[1], cov_ptr + 26 * sizeof(float), sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(&cov_diag[2], cov_ptr + 52 * sizeof(float), sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(&cov_diag[3], cov_ptr + 78 * sizeof(float), sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuMemcpyDtoHAsync(&cov_diag[4], cov_ptr + 104 * sizeof(float), sizeof(float), cu_s->str));
    CHECK_CUDA(cu_f, cuStreamSynchronize(cu_s->str));
    FILE *logf = fopen("/tmp/tmp.itUGhU69br/libvmaf/debug.log", "a");
    if (logf) {
      fprintf(logf,
              "{\"hypothesisId\":\"A_B_C\",\"location\":\"speed_cuda.c:extract_channel\","
              "\"message\":\"GPU est_params results\",\"data\":{"
              "\"num_blocks\":%d,\"num_blocks_h\":%d,\"num_blocks_v\":%d,"
              "\"elements_in_block\":%d,"
              "\"submatrix_w\":%d,\"submatrix_h\":%d,"
              "\"truncated_w\":%d,\"truncated_h\":%d,"
              "\"stride_px\":%d,"
              "\"cov_diag\":[%f,%f,%f,%f,%f],"
              "\"eigenvalues\":[%f,%f,%f,%f],"
              "\"ref_variances\":[%f,%f,%f,%f],"
              "\"ref_entropies\":[%f,%f,%f,%f],"
              "\"dis_variances\":[%f,%f,%f,%f],"
              "\"dis_entropies\":[%f,%f,%f,%f]"
              "},\"timestamp\":%ld}\n",
              ...
              stride_px,
              cov_diag[0], cov_diag[1], cov_diag[2], cov_diag[3], cov_diag[4],
              eigenvalues[0], eigenvalues[1], eigenvalues[2], eigenvalues[3],
              ref_variances[0], ref_variances[1], ref_variances[2], ref_variances[3],
              ref_entropies[0], ref_entropies[1], ref_entropies[2], ref_entropies[3],
              dis_variances[0], dis_variances[1], dis_variances[2], dis_variances[3],
              dis_entropies[0], dis_entropies[1], dis_entropies[2], dis_entropies[3],
              (long)time(NULL));
      fclose(logf);
    }
  }
  ...
  // #region agent log - Hypothesis D: GPU score computation
  {
    CHECK_CUDA(cu_f, cuStreamSynchronize(cu_s->str));
    float score_sum = bufs->score_host[score_slot];
    float base_entropy = dim->elements_in_block * 
        (log2f((1.0f + speed_options->speed_nn_floor) * sigma_nn) + log2f(2.0f * M_PI * M_E));
    FILE *logf = fopen("/tmp/tmp.itUGhU69br/libvmaf/debug.log", "a");
    if (logf) {
      fprintf(logf,
              "{\"hypothesisId\":\"D\",\"location\":\"speed_cuda.c:extract_channel\","
              "\"message\":\"GPU score computation\",\"data\":{"
              "\"score_sum\":%f,\"num_blocks\":%d,\"final_score\":%f,"
              "\"base_entropy\":%f,\"sigma_nn\":%f,\"nn_floor\":%f,"
              "\"weight_var_mode\":%d"
              "},\"timestamp\":%ld}\n",
              score_sum, dim->num_blocks, score_sum / dim->num_blocks,
              base_entropy, sigma_nn, (float)speed_options->speed_nn_floor,
              speed_options->speed_weight_var_mode,
              (long)time(NULL));
      fclose(logf);
    }
  }
